...
MATH_REPO = {
    'nepal': {
        ...
        'related': [
            {"raw": "SSF_Total = 31% of Basic", "title": "Total Social Security Contribution", "desc": "Includes 20% employer contribution for medical and pension fund."},
            {"raw": "Taxable_Income = Gross - (Retirement_Contribution + Insurance_Premium)", "title": "Adjusted Taxable Income", "desc": "The amount used to determine which tax slab applies."}
        ]
    },
    'finance': {
        ...
        'related': [
            ...
            {"raw": "EAR = (1 + r/n)^n - 1", "title": "Effective Annual Rate", "desc": "The true annual cost of borrowing or return on investment."}
        ]
    },
    'health': {
        ...
        'related': [
            {"raw": "BMR (Male) = 10W + 6.25H - 5A + 5", "title": "Mifflin-St Jeor Equation", "desc": "Calculates Basal Metabolic Rate for men based on Weight (W), Height (H), and Age (A)."},
            {"raw": "IBW (Male) = 50kg + 2.3kg per inch over 5ft", "title": "Devine Formula for Ideal Weight", "desc": "Clinical standard for determining target weight ranges."}
        ]
    }
}
...
for cid, name, cat in calculators:
  ...
  '{cid}': {{
    ...
    content: (
      <>
        <div className="nepalc-mathematical-lab text-[16px] text-slate-800 leading-relaxed bg-white p-4">
          ...
          <div className="bg-white border border-slate-200 p-8 rounded-xl mb-8 shadow-sm">
            ...
            <div className="border-t pt-4">
                <h4 className="font-bold text-slate-800 mb-2">Parameter Breakdown & Explanation:</h4>
          ...
          <h2 className="text-xl font-bold mb-4 text-slate-900 uppercase tracking-wide">2. Detailed Computational Walkthrough</h2>
          <div className="space-y-4 mb-10 text-slate-700">
            <p><strong>Step 1 (Data Entry):</strong> Initialize the engine by providing the base values. Each parameter must be in the correct unit (e.g., annual interest rates should be entered as whole percentages, which the engine converts to decimals).</p>
            <p><strong>Step 2 (Normalization & Scaling):</strong> Our engine normalizes various time periods (monthly vs annually) to ensure they align with the compounding frequency (n). This step is critical to prevent rounding errors in long-term projections.</p>
            <p><strong>Step 3 (The Iterative Cycle):</strong> For tools involving periodic contributions, the engine performs a recursive summation or applies the standard annuity formula to calculate the accumulated value over the investment length (t).</p>
            <p><strong>Step 4 (Final Audit):</strong> The raw result is adjusted for any statutory variables (such as tax shields or inflation indexes) to provide the high-precision output seen on your screen.</p>
          </div>

          <h2 className="text-xl font-bold mb-4 text-slate-900 uppercase tracking-wide">3. Related Mathematical Models & Variations</h2>
          <p className="mb-6 text-slate-600 italic">Advanced users can utilize these related formulas for manual verification or alternative statistical projections.</p>
