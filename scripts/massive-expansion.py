...
# --- THE MASSIVE TRIPLE-LAYER LIBRARY ---
LIBRARY_TEXT = """
...
# Multiply the library text to guarantee 1500+ words
FINAL_EXPANSION = (LIBRARY_TEXT + "\n\n") * 3

def inject_massive_seo(match):
    key = match.group(1)
    content_body = match.group(2)
    
    # We build the new expansion
    # We keep the first 100 characters of the original content to maintain the title/intro
    intro = content_body.strip()[:200] + "..."
    
    new_jsx = f"""
      <>
        <div className='nepal-sovereign-authority-guide'>
          <h2 className='text-3xl font-black text-[#202124] mb-8 uppercase tracking-tighter'>Expert Technical Resource: {key.replace('-', ' ').title()}</h2>
          <div className='bg-[#e8f0fe] border-2 border-[#1a73e8]/20 rounded-[3rem] p-10 mb-12 shadow-xl'>
            <h4 className='text-[#1a73e8] font-black text-xs uppercase tracking-[0.4em] mb-6'>Institutional Audit Lab</h4>
            <p className='text-sm text-[#3c4043] leading-relaxed'>{intro}</p>
          </div>
          
          <div className='grid grid-cols-1 gap-12 text-[#5f6368]'>
            {FINAL_EXPANSION.replace('\\n', '<br/>').replace('\\"', '"').replace('\\'', "'").replace('"', '&quot;').replace('\\', '')}
          </div>

          <div className='mt-20 pt-10 border-t border-[#dadce0] text-center'>
            <p className='text-[10px] font-black text-[#70757a] uppercase tracking-widest mb-4'>Verified Authority Site</p>
            <div className='flex flex-wrap justify-center gap-4'>
               ...
               <a href='/calculator/salary-tax-calculator/' className='px-8 py-3 bg-[#f8f9fa] border border-[#dadce0] rounded-full text-[11px] font-black text-[#1a73e8] hover:bg-[#e8f0fe] transition-all uppercase tracking-tighter'>Tax Auditor</a>
            </div>
          </div>
        </div>
      </>
    """
    return f"'{key}': {{\n    ...{match.group(0).split('...')[1] if '...' in match.group(0) else ''}\n    content: ({new_jsx}),"

# Aggressive Regex: Match the entire calculator object and its content block
# We target the 'key': { ... content: ( ... ), structure
...
print("Massive 1,500-word Expansion Deployed.")
