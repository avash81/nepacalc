...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages = [
    ...
    '/calculator/nepali-date/bulk',
  ].map((route) => ({
    url: cleanUrl(route),
    lastModified: (route === '/electricity/nepal-unit-price' || route === '/electricity/nea-tariff-rates') ? new Date('2026-06-19T00:00:00Z') : (route === '/engineering/3d' || route === '/market-rates' || route === '/nepal/nepal-budget') ? new Date() : (route === '/calculator/nepali-date/bulk') ? new Date('2026-08-08T00:00:00Z') : lastModDate,
    changeFrequency: 'weekly' as const,
    priority: route === '' ? 1.0 :
              route === '/engineering/3d' ? 0.95 :
              route === '/directory' ? 0.95 :
              route === '/nepal/nepal-budget' ? 0.95 :
              route === '/calculator/nepali-date/bulk' ? 0.90 :
