...
export function IncomeTaxForm({ state, update, isMonthly, setIsMonthly }: Props) {
  ...
  const InputGroup = ({ label, helper, value, onChange, type = "number" }: any) => (
      ...
      <div className="flex items-center gap-1.5">
        ...
        {helper && (
          <div className="group relative cursor-help">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block w-48 p-2 bg-[#202124] text-white text-[11px] rounded shadow-lg z-10 text-center pointer-events-none">
              {helper}
  ...
  return (
    <div className="bg-white border border-[#DADCE0] rounded-xl shadow-sm overflow-hidden flex flex-col gap-6 p-6">
      ...
      {/* SECTION 1: INCOME */}
      <div>
        <h3 className="text-sm font-black text-[#202124] border-b border-[#DADCE0] pb-2 mb-4">1. Income</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1">
            <div className="flex justify-between items-center">
              <label className="text-xs font-bold text-[#5F6368] uppercase tracking-wider">{isMonthly ? "Monthly Salary" : "Annual Salary"}</label>
          ...
          <InputGroup label="Annual Bonus" value={state.annualBonus} onChange={(v: number) => update({ annualBonus: v })} helper="Festival bonus, Dashain bonus, or performance bonus" />
        </div>
      </div>
