...
export default function Calculator() {
  const [state, setState] = useSyncState('matrix_calc_v4', DEFAULT_STATE);
  const { mode, a11, a12, a13, a21, a22, a23, a31, a32, a33 } = state;
  const updateState = (u: Partial<typeof DEFAULT_STATE>) => setState({ ...state, ...u });

  // ── CORE CALCULATOR LOGIC (PRESERVED) ──────────────────────────────────
  const result = useMemo(() => {
    ...
    return { det, trace, inv, trans };
  }, [mode, a11, a12, a13, a21, a22, a23, a31, a32, a33]);

  const inputCls = "w-full h-14 border border-[#DADCE0] bg-white font-mono font-black text-xl focus:border-[#1A73E8] focus:ring-1 focus:ring-[#1A73E8] outline-none text-center transition-all rounded-xl shadow-inner text-[#202124]";

  let copyText = '';
  if (result.det !== undefined) {
    copyText += `Determinant: ${result.det}\n`;
    copyText += `Trace: ${result.trace}\n`;
    if (result.inv) {
      copyText += `Inverse:\n`;
      result.inv.forEach(row => { copyText += `[${row.map(n => n.toFixed(3)).join(', ')}]\n`; });
    }
  }
  ...
  return (
    <ModernCalcLayout
      slug="matrices"
      crumbs={[{ label: 'Engineering Mathematics', href: '/engineering/' }, { label: 'Matrix Calculator' }]}
      ...
      results={
        <div className="space-y-6">
          <div className="bg-white border border-[#DADCE0] rounded-lg overflow-hidden text-center shadow-sm">
             {/* SECTION 1: RESULT PANEL WITH COPY BUTTON */}
             <div className="px-4 py-2.5 bg-[#F8F9FA] border-b border-[#DADCE0] flex items-center justify-between">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-[#202124]">Calculated Results</h3>
             </div>
             
             <div className="p-6 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-[#F8F9FA] rounded border border-[#DADCE0] flex flex-col justify-between">
                    <div>
                      <div className="text-[10px] font-black text-[#70757A] uppercase mb-2">Determinant |A|</div>
                      <div className="mb-3 overflow-x-auto"><InlineMath math={mode === '2x2' ? "|A| = ad - bc" : "|A| = a(ei-fh) - b(di-fg) + c(dh-eg)"} /></div>
                    </div>
                    <div className="text-2xl font-black text-[#1A73E8] font-mono">{Number.isInteger(result.det) ? result.det : result.det.toFixed(4)}</div>
                  </div>
                  <div className="p-4 bg-[#F8F9FA] rounded border border-[#DADCE0] flex flex-col justify-between">
                    <div>
                      <div className="text-[10px] font-black text-[#70757A] uppercase mb-2">Trace tr(A)</div>
                      <div className="mb-3 overflow-x-auto"><InlineMath math={mode === '2x2' ? "\\text{tr}(A) = a + d" : "\\text{tr}(A) = a + e + i"} /></div>
                    </div>
                    <div className="text-2xl font-black text-[#188038] font-mono">{Number.isInteger(result.trace) ? result.trace : result.trace.toFixed(4)}</div>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {result.inv ? (
                    <div>
                      <div className="text-[10px] font-black text-[#70757A] uppercase mb-3 text-left flex justify-between items-center">
                        <span>Inverse Matrix A⁻¹</span>
                        <div className="normal-case font-normal text-[#5F6368] hidden sm:block"><InlineMath math="A^{-1} = \frac{1}{|A|} \text{adj}(A)" /></div>
                      </div>
                      <table className="w-full border-collapse border border-[#DADCE0] bg-white shadow-sm rounded-lg overflow-hidden">
                        <tbody>
                          {result.inv.map((row, i) => (
                            <tr key={i}>
                              {row.map((v, j) => (
                                <td key={j} className="border border-[#E8EAED] p-3 font-mono text-sm font-black text-[#202124] text-center bg-[#F8F9FA] min-w-[60px]">
                    ...
                    <div className="p-4 bg-[#FCE8E6] border border-[#FAD2CF] rounded text-center flex flex-col justify-center h-full">
                      <div className="text-xs font-black uppercase text-[#D93025] mb-2 flex items-center justify-center gap-1.5">
                        <AlertTriangle className="w-3.5 h-3.5" /> Singular Matrix
                      </div>
                      <div className="text-[11px] text-[#202124] font-bold">This matrix cannot be inverted because its determinant equals zero.</div>
                    </div>
                  )}

                  <div>
                    <div className="text-[10px] font-black text-[#70757A] uppercase mb-3 text-left flex justify-between items-center">
                      <span>Transposed Matrix A^T</span>
                      <div className="normal-case font-normal text-[#5F6368] hidden sm:block"><InlineMath math="A^T_{ij} = A_{ji}" /></div>
                    </div>
                    <table className="w-full border-collapse border border-[#DADCE0] bg-white shadow-sm rounded-lg overflow-hidden">
                      <tbody>
                        {result.trans.map((row, i) => (
                          <tr key={i}>
                            {row.map((v, j) => (
                              <td key={j} className="border border-[#E8EAED] p-3 font-mono text-sm font-black text-[#202124] text-center bg-white min-w-[60px]">
                                {Number.isInteger(v) ? v : v.toFixed(3)}
                              </td>
                            ))}
                          </tr>
      ...
      details={step1Structure}
      /* Removed faqs prop to strictly follow Step 1 */
    />
  );
}
