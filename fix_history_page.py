...
bs_min = sorted_records[0].get('date_bs', '')
bs_max = sorted_records[-1].get('date_bs', '')

# Compute coverage ranges for verified records only? No, for all records or verified?
# The user wants 'Verified Coverage Ranges', so let's only look at verified.
...
if v_dates:
    ...
    for i in range(1, len(v_dates)):
        ...
        # If it's the next day or weekend gap (e.g. up to 3 days gap)
        if (d - last_d).days <= 3:
            last_d = d
            last_ad = v_dates[i]
            count += 1
...
import re
# Replace Data Summary block
page = re.sub(
    r'\{/\* Data Summary — below table \*/\}.*?</ul>\s*</div>',
    replacement_code.replace('\\', '\\\\'), # naive escaping
    page,
    flags=re.DOTALL
)

# Move Related Tools below FAQ
# First extract Related Tools
related_match = re.search(r'(\s*\{/\* Related tools \*/\}.*?</section>)', page, flags=re.DOTALL)
if related_match:
    related_block = related_match.group(1)
    # Remove from original
    page = page.replace(related_block, '')
    # Find end of FAQ section
    faq_end_match = re.search(r'\{/\* FAQ Section \*/\}.*?</section>', page, flags=re.DOTALL)
    if faq_end_match:
        faq_end_str = faq_end_match.group(0)
        # Insert after FAQ
        page = page.replace(faq_end_str, faq_end_str + related_block)
