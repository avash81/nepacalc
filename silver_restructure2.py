...
# -----------------
# Extract Market Snapshot and surrounding content (Today's Silver Price)
...
# Extract PricePerformanceWidget and SilverHistoricalData from seo
ppw_regex = r'(<PricePerformanceWidget.*?\/>)'
ppw_match = re.search(ppw_regex, seo, re.DOTALL)
if ppw_match:
    ppw_block = ppw_match.group(1)
    seo = seo.replace(ppw_block, '')
else:
    ppw_block = ""
    print("PPW block not found!")
...
# Remove the <h2 id="todays-silver-price"> and the intro paragraphs
...
# Remove Historical Silver Price Trends from SeoSections
...
# Replace calculator block with Anchor + PPW + History
old_calc_regex = r'<div id="calculator" className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8 scroll-mt-24">.*?<SilverCalculatorClient silverPerTola=\{currentSilver\} />\n                  </div>\n                </div>'
old_calc_match = re.search(old_calc_regex, page, re.DOTALL)

anchor_calc = '''<h2 id="silver-calculator" className="text-2xl font-black text-slate-900 tracking-tighter mb-4 scroll-mt-24">Quick Silver Valuation Calculator</h2>
              <div className="mb-12">
                <a
...
if old_calc_match:
    page = page.replace(old_calc_match.group(0), anchor_calc)
    
    # We must insert snapshot_block BEFORE anchor_calc
    # Find anchor_calc and insert before it
    injection = '<div className="prose prose-slate max-w-none mt-6 mb-8">\n' + snapshot_block + '\n</div>\n\n'
    page = page.replace(anchor_calc, injection + anchor_calc)
    
    # Insert ppw_block and <SilverHistoricalData /> AFTER anchor_calc, meaning right before the Grid.
    ...
    page = page.replace(grid_marker, ppw_block + '\n              <div className="mb-12">\n                <SilverHistoricalData />\n              </div>\n\n          ' + grid_marker)
else:
    print("Old calculator block not found!")
