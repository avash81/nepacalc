...
if idx_int != -1 and idx_ex != -1 and idx_daily != -1 and idx_next != -1:
    ...
    if behave_end in before:
        ...
        print("Behave end not found")
        seo = before + daily_block + ex_block + int_block + after
else:
    print("Comment markers not found")
...
if quick_answer_marker in page:
    ...
    print("Quick answer marker not found")

# Replace Calculator block
calc_regex = r'<div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8">.*?<SilverCalculatorClient silverPerTola=\{currentSilver\} />\s*</div>\s*</div>'
