...
match1 = re.search(r'\{\/\*\s*── Market Insight ──\s*\*\/\}.*?(?=<section>)', content, re.DOTALL)
idx2 = content.find('export default function LiveGoldPricePage()')
idx3 = content.find('export default function Page()')
idx4 = content.rfind('</main>')

end_idx = idx2 if idx2 != -1 else idx3
if end_idx == -1: end_idx = idx4

if match1:
    idx1 = match1.start()
    old_section = content[idx1:end_idx]
...
'''
    content = content[:idx1] + new_section + content[end_idx:]
